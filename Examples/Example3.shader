Shader "Thry/Example 3"
{
	Properties 
  { 
      [HideInInspector] shader_is_using_thry_editor("", Float)=0
		[HideInInspector] shader_master_label("<color=#ff0000ff>Example 3</color>", Float) = 0
		...
		[HideInInspector] footer_website("{text:Website,action:{type:URL,data:https://www.poiyomi.com/},hover:WEBSITE}", Float) = 0
		[HideInInspector] footer_github("{text:GitHub,action:{type:URL,data:https://github.com/Thryrallo/ThryEditor},hover:GITHUB}", Float) = 0
		...
		_EmissionStrength("Emission Strength", Range(0, 20)) = 0
			...
			[HideInInspector] m_end_angularFade("Angular Fade", Float) = 0
				...
				[Toggle(_)]_IgnoreFog("Ignore Fog", Float) = 0
					...
					[ByteSlider] _StencilReadMask("Stencil ReadMask Value", Range(0, 255)) = 255
					[ByteSlider] _StencilWriteMask("Stencil WriteMask Value", Range(0, 255)) = 255
					...
					[HideInInspector] m_start_stencil_calculator("Stencil Calculator--{persistent_expand:true,default_expand:false}", Float) = 0
					[HideInInspector] s_start_stencil_grid("Calculator--{persistent_expand:true,default_expand:false}", Float) = 0
					//Two-sided shader? [ThryStencilCalculator(Back)] reads _StencilBackCompareFunction, _StencilBackPassOp,
					//_StencilBackFailOp, _StencilBackZFailOp and _StencilBackCheckResult instead. Front works the same way.
					[HideInInspector][IntRange] _StencilBufferValue("Existing Buffer Value", Range(0, 255)) = 0
