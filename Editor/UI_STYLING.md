...
Poi/Thry/Settings → Theme offers independent Dark gray, Medium gray, and Light gray sliders from −20 to 20, plus dropdowns for 11–14 px text and independent 18/20/22 px property heights and 18/20/22/24 px header heights. Gray sliders use broader neutral ranges for each Unity skin, with foreground adjustments at the extremes. Enabled text adapts to the extended range; the gray controls can also bring neighboring surfaces closer together. Reset appearance restores the default shades, 12 px text, 18 px property height, and 22 px header height. Nested headers are 2 px shorter than the selected header height with an 18 px minimum.
...
- Preset and animation markings use Monokai cyan for P, purple for PA (animation-only preset content), green for A, and yellow for RA. Animation labels and header dots share these colors, with darker counterparts for the light skin. Preset colors live in `--thry-preset-color` and `--thry-preset-animation-color`; animation colors and IMGUI equivalents live in `Styles.cs`.
- Shader-lock, Presets, rendering mode, and all other filled action buttons share the darker action surface and hover colors (`--thry-action-background` and `--thry-action-hover-background`), including decal positioning and Bake Color Adjust. The title and search strip, search strip and action block, and action block and first section are separated by 4px gaps. Gaps within the action block remain 2px, including between Presets and rendering mode. Search and toolbar controls use a 22px control height, with 20px inner search fields and unchanged 18px toolbar artwork.
