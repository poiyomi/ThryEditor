                    ...
                    case "ThryMultiFloatButtons": case "ThryMultiFloats":
                        ...
                        for(int i=0;i<ids.Length;i++)
                        {
                            ...
                            if(tiles) { string defaultLabel = Drawers.TileLabelUtility.FormatTileLabel(attribute.Args[i]);
                                var button=new Button(()=>{if(!Model.CanEdit(target)) return; Model.Number(target,target.MaterialProperty.hasMixedValue||target.MaterialProperty.GetNumber()<=.5f?1:0); if (!Drawers.TileLabelUtility.IsUdimProperty(target.MaterialProperty.name)) foreach(var id in ids) { ShaderProperty linked; if(Model.Shader.PropertyDictionary.TryGetValue(id,out linked)) linked.SetAnimated(target.IsAnimated,target.IsRenaming); }}) {text=defaultLabel,name="tile-"+target.MaterialProperty.name}; button.style.flexGrow=1;
