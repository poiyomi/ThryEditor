...
namespace Thry.ThryEditor.Drawers
{
	...
	public class Ramp4Drawer : MaterialPropertyDrawer
	{
		...
		private void DrawRampPreview(Rect rect, Vector4 ramp)
		{
			...
			// Background
			EditorGUI.DrawRect(rect, Colors.graphBackground);
			...
			EditorGUI.DrawRect(new Rect(rect.x, rect.y, handleMargin, rect.height), Colors.graphGutter);
			EditorGUI.DrawRect(new Rect(rect.xMax - handleMargin, rect.y, handleMargin, rect.height), Colors.graphGutter);
			...
			if (_normalized && (minVal < 0f || maxVal > 1f))
			{
				...
				Handles.color = Colors.referenceLine;
			...
			Handles.color = Colors.handleOrange;
			float leftX = Mathf.Clamp(rect.x + handleMargin * 0.5f, rect.x, rect.xMax);
			...
			Handles.color = Colors.handleBlue;
			float rightX = Mathf.Clamp(rect.xMax - handleMargin * 0.5f, rect.x, rect.xMax);
			...
			// Border
			Handles.color = Colors.borderDark;
		...
		private void HandlePreviewDrag(Rect rect, ref float t0, ref float t1, ref float v0, ref float v1, MaterialProperty prop)
		{
			...
			Rect hv0 = new Rect(rect.x, rect.y, handleMargin, rect.height);
			Rect hv1 = new Rect(rect.xMax - handleMargin, rect.y, handleMargin, rect.height);
